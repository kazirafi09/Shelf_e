...
<div class="container px-4 py-6 mx-auto md:py-8 max-w-7xl" x-data="{ mounted: false }" x-init="setTimeout(() => mounted = true, 100)">
    ...
    @if($quote)
        <div class="mt-8 italic text-center text-muted-foreground"
             x-data="{ 
        ...
        <div class="relative flex justify-center items-center w-full h-[400px] md:h-[500px] cursor-grab active:cursor-grabbing select-none" 
            ...
            <template x-for="(slide, index) in slides" :key="index">
                <div class="absolute flex flex-col items-center transition-all ease-out"
                    ...
                    {{-- The Image Card --}}
                    <div class="w-[260px] h-[340px] md:w-[320px] md:h-[420px] transition-shadow duration-300 bg-center bg-cover border border-border shadow-xl cursor-pointer rounded-none hover:shadow-2xl"
                        @click="goTo(index)"
                    ...
                    <div class="mt-6 text-center transition-opacity duration-500" 
                        ...
                        <h3 class="mt-1 text-xl font-bold text-foreground" x-text="slide.title"></h3>
                    </div>
    ...
    <div class="grid grid-cols-1 gap-4 mt-8 transition-all duration-700 ease-out delay-300 transform md:grid-cols-4" :class="mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'">
        <div class="flex items-center p-4 transition-shadow bg-card text-card-foreground border border-border shadow-sm rounded-2xl hover:shadow-md">
        ...
        <div class="flex items-center p-4 transition-shadow bg-card text-card-foreground border border-border shadow-sm rounded-2xl hover:shadow-md">
            ...
            <div class="ml-4">
                <h4 class="text-sm font-bold text-foreground">Original Books</h4>
                <p class="text-[10px] text-muted-foreground uppercase tracking-wider">Guaranteed authentic</p>
            </div>
        </div>
    </div>
    ...
    <div class="mt-20 transition-all duration-700 ease-out delay-700 transform" :class="mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'">
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
            <a href="{{ route('categories.index') }}" class="relative p-8 overflow-hidden transition-colors bg-card text-card-foreground border border-border shadow-sm rounded-3xl group hover:border-cyan-500 hover:shadow-lg">
            ...
            <a href="#" class="relative p-8 overflow-hidden transition-colors bg-card text-card-foreground border border-border shadow-sm rounded-3xl group hover:border-cyan-500 hover:shadow-lg">
    ...
    <div class="mt-20 transition-all duration-700 ease-out delay-700 transform" :class="mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <a href="{{ route('categories.index', ['authors[]' => $item->author]) }}" class="flex items-center p-5 transition-all duration-300 bg-card text-card-foreground border border-border shadow-sm cursor-pointer rounded-2xl hover:shadow-md hover:-translate-y-1 group">
                ...
                <div class="ml-5 overflow-hidden">
                    <h3 class="text-lg font-bold text-foreground truncate transition-colors duration-300 group-hover:text-cyan-600" title="{{ $item->author }}">{{ $item->author }}</h3>
            ...
            <div class="relative flex flex-col p-4 overflow-hidden transition-all duration-500 bg-card text-card-foreground border border-border rounded-2xl group hover:shadow-xl hover:-translate-y-2 hover:border-cyan-100">
                ...
                <a href="/product/{{ $book->slug }}" class="z-10 block grow">
                    ...
                    <div class="flex items-center mb-1.5 space-x-1">
                        ...
                        <span class="text-xs font-bold text-foreground">{{ $book->rating }}</span>
                    </div>
