...
@section('subtitle', "Editing voucher: {{ $voucher->code }}")
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <input type="number" id="min_order_amount" name="min_order_amount"
                       value="{{ old('min_order_amount', $voucher->min_order_amount) }}" min="0" step="1"
                       class="block w-full px-4 py-3 text-sm bg-background border border-input text-foreground focus:ring-2 focus:ring-ring focus:outline-none rounded-[var(--radius)] shadow-sm">
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" id="max_uses" name="max_uses"
                           value="{{ old('max_uses', $voucher->max_uses) }}" min="1" placeholder="Unlimited"
                           class="block w-full px-4 py-3 text-sm bg-background border border-input text-foreground focus:ring-2 focus:ring-ring focus:outline-none rounded-[var(--radius)] shadow-sm">
                    <p class="mt-1 text-xs text-muted-foreground">Leave blank for unlimited.</p>
                </div>
                <div>
                    ...
                    <input type="number" id="max_uses_per_user" name="max_uses_per_user"
                           value="{{ old('max_uses_per_user', $voucher->max_uses_per_user) }}" min="1" required
                           class="block w-full px-4 py-3 text-sm bg-background border border-input text-foreground focus:ring-2 focus:ring-ring focus:outline-none rounded-[var(--radius)] shadow-sm">
                </div>
            </div>
            ...
            <div class="p-4 bg-muted rounded-xl">
                <p class="text-sm font-semibold text-foreground">Usage: <span class="font-bold">{{ $voucher->used_count }}</span> times used</p>
            </div>
            ...
            <div class="flex flex-col gap-4 pt-2">
                ...
                <label class="flex items-center gap-3 cursor-pointer">
                    ...
                    <input type="checkbox" name="is_announced" value="1"
                           {{ old('is_announced', $voucher->is_announced) ? 'checked' : '' }}
                           class="w-4 h-4 rounded border-input text-primary focus:ring-ring">
