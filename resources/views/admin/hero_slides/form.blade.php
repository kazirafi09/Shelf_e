...
<div class="max-w-3xl mx-auto">
    ...
    <div class="p-8 bg-card text-card-foreground border border-border shadow-xl rounded-3xl">
        <form action="{{ isset($heroSlide) ? route('admin.hero-slides.update', $heroSlide) : route('admin.hero-slides.store') }}" 
            ...
            <div class="p-6 border-2 border-border border-dashed rounded-2xl bg-muted/50">
                <label class="block mb-4 text-sm font-bold tracking-wide text-foreground uppercase">Slide Cover Image</label>
                
                @if(isset($heroSlide))
                    <div class="mb-4">
                        <p class="mb-2 text-xs font-semibold text-muted-foreground">Current Image:</p>
                ...
                <p class="mt-3 text-xs text-muted-foreground">Recommended size: 800x1000px. Supports JPG, PNG, WebP.</p>
            ...
            <div class="grid grid-cols-1 gap-8 md:grid-cols-2">
                {{-- Tag Input --}}
                <div>
                    <label class="block mb-2 text-sm font-bold tracking-wide text-foreground uppercase">Tag (e.g. #01)</label>
                    <input type="text" name="tag" value="{{ old('tag', $heroSlide->tag ?? '') }}"
                           placeholder="#01"
                           class="w-full px-5 py-3 bg-background border border-input text-foreground focus:ring-2 focus:ring-ring focus:outline-none rounded-[var(--radius)] transition-all">
                </div>

                {{-- Sort Order Input --}}
                <div>
                    <label class="block mb-2 text-sm font-bold tracking-wide text-foreground uppercase">Sort Order</label>
            ...
            {{-- Title Input --}}
            <div>
                <label class="block mb-2 text-sm font-bold tracking-wide text-foreground uppercase">Slide Title</label>
            ...
            {{-- Action Buttons --}}
            <div class="flex flex-col-reverse gap-4 pt-6 border-t border-border md:flex-row">
                <a href="{{ route('admin.hero-slides.index') }}"
                   class="flex items-center justify-center px-8 py-4 text-sm font-bold text-muted-foreground transition-colors bg-muted rounded-xl hover:opacity-80">
                    Discard Changes
                </a>
                <button type="submit" 
