...
<div class="flex items-center justify-center min-h-[calc(100vh-200px)] py-12 px-4 sm:px-6 lg:px-8">
    <div class="w-full max-w-md p-8 space-y-8 bg-white border border-gray-100 shadow-2xl rounded-3xl md:p-10 ring-1 ring-gray-900/5">
        ...
        <div class="text-center">
            <div class="inline-flex items-center justify-center w-16 h-16 mb-6 text-gray-500 rounded-full bg-gray-50">
        ...
        <form method="POST" action="{{ route('password.store') }}" class="mt-8 space-y-5">
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input id="email" type="email" name="email" value="{{ old('email', request()->email) }}" required autofocus autocomplete="username" class="block w-full py-3.5 pl-11 pr-4 text-sm text-gray-500 bg-gray-100 border-gray-200 rounded-xl shadow-sm focus:ring-0 cursor-not-allowed" readonly />
                </div>
                <x-input-error :messages="$errors->get('email')" class="mt-2" />
            </div>

            <div>
                <label for="password" class="block mb-1.5 text-sm font-bold text-gray-900">New Password</label>
            ...
            <div>
                <label for="password_confirmation" class="block mb-1.5 text-sm font-bold text-gray-900">Confirm New Password</label>
                <div class="relative" x-data="{ show: false }">
                    <div class="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none">
                        <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path></svg>
                    </div>
                    <input id="password_confirmation" :type="show ? 'text' : 'password'" name="password_confirmation" placeholder="Repeat New Password" required autocomplete="new-password" class="block w-full py-3.5 pl-11 pr-12 text-sm placeholder-gray-400 transition-colors border-gray-200 rounded-xl shadow-sm bg-gray-50 focus:bg-white focus:border-gray-500 focus:ring-2 focus:ring-gray-500/20" />
            ...
            <div class="pt-2">
                <button type="submit" class="w-full px-6 py-3.5 text-sm font-bold text-white transition-all bg-gray-700 rounded-xl shadow-lg hover:bg-gray-800 hover:shadow-xl hover:-translate-y-0.5 active:translate-y-0 active:scale-95">
                    Reset Password
                </button>
            </div>
        </form>
