...
<div class="flex items-center justify-center min-h-[calc(100vh-200px)] py-12 px-4 sm:px-6 lg:px-8">
    <div class="w-full max-w-md p-8 space-y-8 bg-card text-card-foreground border border-border shadow-2xl rounded-3xl md:p-10">
        ...
        <form method="POST" action="{{ route('password.email') }}" class="mt-8 space-y-6">
            ...
            <div class="pt-2">
                <button type="submit" class="w-full px-6 py-3.5 text-sm font-bold text-white transition-all bg-gray-700 rounded-xl shadow-lg hover:bg-gray-800 hover:shadow-xl hover:-translate-y-0.5 active:translate-y-0 active:scale-95 flex justify-center items-center">
                    ...
                    <svg class="w-4 h-4 ml-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg>
                </button>
            </div>

            <div class="pt-4 text-center">
                <a href="{{ route('login') }}" class="text-sm font-bold text-gray-500 transition-colors hover:text-gray-800">
                    &larr; Back to Login
                </a>
            </div>
        </form>
        ...
        <div class="pt-6 mt-8 text-xs text-center text-gray-400 border-t border-gray-100">
            If you're having trouble receiving the email, please check your spam folder or <a href="{{ Route::has('contact') ? route('contact') : '#' }}" class="transition-colors hover:text-gray-700 hover:underline">contact support</a>.
        </div>
    </div>
</div>
@endsection