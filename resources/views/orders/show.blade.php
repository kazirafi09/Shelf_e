...
<div class="container max-w-5xl px-4 py-8 mx-auto">
    ...
    <div class="grid grid-cols-1 gap-8 md:grid-cols-3">
        <div class="p-6 bg-card text-card-foreground border border-border shadow-sm md:col-span-2 rounded-2xl">
            <h3 class="pb-3 mb-4 text-lg font-bold text-foreground border-b border-border">Items Ordered</h3>
            <div class="space-y-4">
                @foreach($order->items as $item)
                <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-4">
                        <div class="flex items-center justify-center w-12 h-16 bg-muted rounded shrink-0">
                        ...
                        <div>
                            <h4 class="font-bold text-foreground">{{ $item->product ? $item->product->title : 'Product no longer available' }}</h4>
                            <p class="text-sm text-muted-foreground">Qty: {{ $item->quantity }} x ৳{{ number_format($item->price, 2) }}</p>
                        </div>
                    </div>
                    <span class="font-bold text-foreground">৳{{ number_format($item->price * $item->quantity, 2) }}</span>
                </div>
                @endforeach
            </div>
        </div>
        ...
        <div class="space-y-6">
            <div class="p-6 border border-border bg-muted rounded-2xl">
                <h3 class="pb-3 mb-4 text-lg font-bold text-foreground border-b border-border">Summary</h3>
            ...
            <div class="p-6 bg-card text-card-foreground border border-border shadow-sm rounded-2xl">
                <h3 class="pb-3 mb-4 text-lg font-bold text-foreground border-b border-border">Delivery Address</h3>
                <address class="space-y-1 text-sm not-italic text-muted-foreground">
                    <p class="font-bold text-foreground">{{ $order->name }}</p>
                    <p>{{ $order->phone }}</p>
